...
const login = (_a) => __awaiter(void 0, [_a], void 0, function* ({ email, password, }) {
    ...
    const staff = yield user_model_1.default.findOne({
        email,
    ...
    console.log("Hash in DB:", staff.password);
    //Kiểm tra mật khẩu
    ...
    //login thành công
    //Tạo token
    const accessToken = jsonwebtoken_1.default.sign({ _id: staff._id, email: staff.email }, env_helper_1.env.JWT_SECRET, {
        expiresIn: "24h", // expires in 24 hours (24 x 60 x 60)
    });
    const refreshToken = jsonwebtoken_1.default.sign({ _id: staff._id, email: staff.email }, env_helper_1.env.JWT_SECRET, {
